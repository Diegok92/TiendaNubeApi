...
const postImages2 = (req, res) => {
  ...
  fs.createReadStream("app/assets/documents/SKUs.csv") // Cambia 'input.csv' al nombre de tu archivo CSV
    ...
    .on("end", () => {
      postImages2(id_productos[0], SKUs[0], 0);
    });

  console.log("SKUS: " + SKUs);

  function postImages2(id_producto, SKU, index) {
    ...
    if (index < id_productos.length) {
      ...
      if (imgBase64 != false) {
        setTimeout(
          () => postImages2(id_productos[index], SKUs[index], index),
          TIME_RANGE
        );
  ...
  res.send("Fin de la carga de imagenes");
};
