...
const deleteImages2 = (req, res) => {
  ...
  fs.createReadStream("app/assets/documents/SKUs.csv") // Cambia 'indelete.csv' al nombre de tu archivo CSV
    ...
    .on("end", () => {
      deleteImages2(id_productos[0], SKUs[0], 0);
    });

  console.log("SKUS: " + SKUs);

  function deleteImages2(id_producto, SKU, index) {
    ...
    if (index < id_productos.length) {
      ...
      if (imgBase64 != false) {
        setTimeout(
          () => deleteImages2(id_productos[index], SKUs[index], index),
          TIME_RANGE
        );
